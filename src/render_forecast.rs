...
pub fn render_dist(start: &Quantogram, end: &Quantogram, days: &Range<u32>) -> Svg {
    ...
    let max_y = coords.iter().map(|(_, y)| y).fold(0.0, |a, y| y.max(a));
    let hscale = 100.0;

    let mut path: Path = Vec::with_capacity(coords.len() + 3);
    path.push(Segment::Move(0 as Coord, (H - 0.0 * H) as Coord));

    for (day, y) in coords {
        path.push(Segment::Line(
            (day * hscale) as Coord,
            (H - (y / max_y) * H) as Coord,
        ));
    }

    path.push(Segment::Line(
        (days.end as f64 * hscale) as Coord,
        H as Coord,
    ));
    ...
    Svg {
        view_box: (
            (days.start as f64 * hscale) as Coord,
            0,
            (days.end as f64 * hscale) as Coord,
            H as Coord,
        ),
